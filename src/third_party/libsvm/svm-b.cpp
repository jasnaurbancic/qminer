			  ...
			  const svm_parameter& param)
{
  printf("kernel function\n");
	switch(param.kernel_type)
	{
		...
		case RBF:
		{
                        printf("called k_function for rbf\n");
			double sum = 0;
...
svm_model *svm_train(const svm_problem *prob, const svm_parameter *param,
	PNotify DebugNotify, PNotify ErrorNotify)
{
  if (param->kernel_type == RBF) printf("RBF successful?\n");
...
{
        printf("inside svm_predict_values\n");
	int i;
	if(model->param.svm_type == ONE_CLASS ||
	   model->param.svm_type == EPSILON_SVR ||
	   model->param.svm_type == NU_SVR || model->nr_class == 2)
	{       printf("inside if\n");
	...
	else
	{
		...
		for(i=0;i<nr_class;i++)
			vote[i] = 0;
		int p=0;
                printf("nr class before important loop %d\n", nr_class);
		for(i=0;i<nr_class;i++)
			for(int j=i+1;j<nr_class;j++)
			{
				double sum = 0;
		...
		free(start);
		free(vote);
                printf("model->label[vote_max_idx] %f\n", model->label[vote_max_idx]);
		return model->label[vote_max_idx];
	}
}

double svm_predict(const svm_model *model, const svm_node *x)
{       printf("Inside svm_predict\n");
	int nr_class = model->nr_class;
        printf("class %d\n", nr_class);
	double *dec_values;
	if(model->param.svm_type == ONE_CLASS ||
	...
	double pred_result = svm_predict_values(model, x, dec_values);
        printf("pred_result %f %f\n", pred_result, dec_values[0]);
	free(dec_values);
        printf("End svm_predict\n");
	return pred_result;
}
